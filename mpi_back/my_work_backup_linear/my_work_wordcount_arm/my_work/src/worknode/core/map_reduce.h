...
run (D *data, uint64_t count, std::vector<keyval>& result)
{
	...
	{
		dprintf("after merge i %d key %d value %llu\r\n", i, result[i].key, result[i].val);
	}
#if 0
...
void MapReduce<Impl, D, K, V, Container>::run_dispatch (std::vector<keyval>& result)
{
	...
	for(int i = 0; i < result.size(); i++)
	{
		dprintf("In run_dispatch i %d key %d value %llu\r\n", i, result[i].key, result[i].val);
	}
#endif
	...
	int nnprocs;
	int mmproc;
	MPI_Comm_size(static_cast<Impl const*>(this)->newcomm, &nnprocs);
	MPI_Comm_rank(static_cast<Impl const*>(this)->newcomm, &mmproc);

	//printf("in run_dispatch ....before alltoall  %d nnprocs %d mmproc %d\r\n", __LINE__, nnprocs, mmproc);
	for(int i = 0; i < nnprocs; i++)
	{
		//printf("proc%d sendbytes[%d] %d sdispls[%d] %d recvbytes[%d] %d rdispls[%d] %d...\r\n",
		//		mmproc, i, sendbytes[i], i, sdispls[i], i, recvbytes[i], i, rdispls[i]);
	}
	...
	printf("in run_dispatch ....after alltoall  %d\r\n", __LINE__);
	//for debug, check the keyvalue that received.
	char *index = rcv_data_buffer;
	if(1 == 0)
	{
		printf("mmproc is %d recvnum %d\r\n", mmproc, recvnum[0]);
		for(int i = 0; i < recvnum[0]; i++)
		{
			int *key_index = (int*)index;
			int *value_index = (int*)(index + sizeof(int));
			char *key, *value;

			printf("keysize %lu\r\n", *key_index);
			printf("valuesize %lu\r\n", *value_index);

			key = index + 2*sizeof(int);
			value = key + *key_index;
	...
	printf("proc %d run2 process over........\r\n", myproc);
#if 1
	//MPI_Barrier(MPI_COMM_WORLD);
	mapreduce_barrier();
