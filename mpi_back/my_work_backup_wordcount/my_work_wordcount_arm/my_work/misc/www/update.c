...
#define FILESYS_PRE		"cramfsImage_ipnc_dm368"

#ifdef U_DEBUG
#define DBG(fmt, args...)	write(STDOUT_FILENO, out_buf, \
								sprintf(out_buf, fmt, ##args) + 1)
#else
#define DBG(fmt, args...)
#endif
#define OUTPUT(fmt, args...)	write(STDOUT_FILENO, out_buf, \
								sprintf(out_buf, fmt, ##args) + 1)
...
const char file[] =
	...
	"Cache-Control: no-store\r\n\r\n"
        "<HTML><HEAD><TITLE>Firmware Update</TITLE></HEAD>\n"
        "<BODY><H1>File create success</H1>\n"
        "I have create file at /tmp.\r\n"
        "Check it out.\r\n</BODY></HTML>\n";
...
void print_image_hdr (image_header_t *hdr)
{
	...
	OUTPUT("   Image Type:   ");print_type(hdr);
	OUTPUT ("\n   Data Size:    %d Bytes = ", ntohl(hdr->ih_size));
	print_size (ntohl(hdr->ih_size), "\n");
	OUTPUT ("   Load Address: %08x\n"
		"   Entry Point:  %08x\n",
	...
	if (hdr->ih_type == IH_TYPE_MULTI) {
		...
		for (i=0; (len = ntohl(*len_ptr)); ++i, ++len_ptr) {
			OUTPUT ("   Image %d: %8ld Bytes = ", i, len);
			print_size (len, "\n");
		}
	}
}
...
int FlashNand(FileInfo* pFile, char *pBlockName, unsigned long offset)
{
	...
	DBG(cmd);
	if(system(cmd)){
		OUTPUT("Fail on update new firmware\r\n");
	...
	if ((fd = open(pBlockName, O_RDWR)) == -1) {
		OUTPUT("open flash error\r\n");
		ret = FAIL;
		goto EXIT_FLASHNAND;
	}
	/* Fill in MTD device capability structure */
	if (ioctl(fd, MEMGETINFO, &meminfo) != 0) {
		OUTPUT("MEMGETINFO error\r\n");
		close(fd);
		ret = FAIL;
		goto EXIT_FLASHNAND;
	}
	...
	if (!(meminfo.oobsize == 16 && meminfo.oobblock == 512) &&
		...
		OUTPUT("Unknown flash (not normal NAND)\n");
		close(fd);
		ret = FAIL;
		goto EXIT_FLASHNAND;
	}
	...
	if (forcejffs2 || forceyaffs) {
		...
		if (autoplace) {
			OUTPUT("Autoplacement is not possible for legacy -j/-y options\n");
			ret = FAIL;
			goto restoreoob;
		}
		if ((old_oobinfo.useecc == MTD_NANDECC_AUTOPLACE) && !forcelegacy) {
			OUTPUT("Use -f option to enforce legacy placement on autoplacement enabled mtd device\n");
			ret = FAIL;
			goto restoreoob;
		}
		if (meminfo.oobsize == 8) {
    			if (forceyaffs) {
				OUTPUT("YAFSS cannot operate on 256 Byte page size");
				ret = FAIL;
				goto restoreoob;
			}
	...
	if ((!pad) && ((imglen % pagelen) != 0)) {
		OUTPUT("Input file is not page aligned\n");
		ret = FAIL;
		goto restoreoob;
	}
	// Check, if length fits into device
	if ( ((imglen / pagelen) * meminfo.oobblock) > (meminfo.size - offset)) {
		OUTPUT("Image %d bytes, NAND page %d bytes, OOB area %u bytes, device size %u bytes\n",
				imglen, pagelen, meminfo.oobblock, meminfo.size);
		OUTPUT("Input file does not fit into device error\r\n");
		ret = FAIL;
		goto restoreoob;
	}
	...
	while (imglen && (offset < meminfo.size)) {
		...
		while (blockstart != (offset & (~meminfo.erasesize + 1))) {
			...
			do {
			   	if ((ret = ioctl(fd, MEMGETBADBLOCK, &offs)) < 0) {
					OUTPUT("ioctl(MEMGETBADBLOCK) error\r\n");
					ret = FAIL;
					goto restoreoob;
				}
				if (ret == 1) {
		...
		if ((cnt = read(pFile->fd, writebuf, readlen)) != readlen) {
			if (cnt == 0)	// EOF
				break;
			OUTPUT("File I/O error on input file\r\n");
			ret =FAIL;
			goto restoreoob;
		}
		...
		if (writeoob) {
			...
			if (ioctl(fd, MEMWRITEOOB, &oob) != 0) {
				OUTPUT("ioctl(MEMWRITEOOB) error\r\n");
				ret = FAIL;
				goto restoreoob;
			}
...
restoreoob:
	...
	if (imglen > 0) {
		OUTPUT ("Data did not fit into device, due to bad blocks\n");
		ret = FAIL;
	}
#endif
EXIT_FLASHNAND:
...
//	FILE* fp;
	...
	sprintf(cmd, "/usr/sbin/flash_eraseall -j %s > %s/%s\n", KERNELBLOCK,
			TEMP_PATH, TEMP_FILE);
	if(system(cmd)){
		OUTPUT("Fail on erase kernel block\r\n");
		sprintf(cmd, "rm -f %s/%s\n", TEMP_PATH, TEMP_FILE);
		system(cmd);
		return FAIL;
	}
...
int DoKernelUpdate(FileInfo* pFile)
{
	...
	pImage = malloc(len);
	if(pImage == NULL){
		OUTPUT("No enough memory to cache uImage\r\n");
		ret = FAIL;
		goto EXIT_UPDATE_K;
	}
	...
	if((nFileSize = pFile->nLength) < 0){
		OUTPUT("Error on get file size\r\n");
		ret = FAIL;
		goto EXIT_UPDATE_K;
	}
	data = nFileSize;
	if(data != (len + sizeof(hdr))){
		OUTPUT("Unkown data found at tail\r\n");
		ret = FAIL;
		goto EXIT_UPDATE_K;
	}
...
int DoFirmwareUpdate(FileInfo* pFile)
{
	...
	if(ret == SUCCESS){
		OUTPUT("Kernel update success\r\n");
	} else if(ret == UNKOWN){
		if((ret = DoRootFileSysUpdate(pFile)) == SUCCESS){
			OUTPUT("File system update success.\r\n");
			sprintf(cmd, "rm -f %s\n", SYS_FILE);
			system(cmd);
			sprintf(cmd, "rm -f %s\n", LOG_FILE);
			system(cmd);
...
int main(int argc, char **argv)
{
	...
	write(STDOUT_FILENO, html_head, sizeof(html_head));
	if((pFileName = GetFileName(STDIN_FILENO, buffer, sizeof(buffer))) != NULL){
		...
		if(CreateFile(pFileName, STDIN_FILENO, buffer, sizeof(buffer), &tFile)==SUCCESS){
			...
			OUTPUT("Fail to create file.\r\n");
#ifdef U_DEBUG
		fclose(fp);
#endif
	} else {
		OUTPUT("<H1>Unkown file.</H1>\r\n");
	}
	write(STDOUT_FILENO, html_end, sizeof(html_end));
